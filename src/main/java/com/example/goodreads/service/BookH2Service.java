...
public class BookH2Service implements BookRepository {
    ...
    public Book updateBook(int bookId, Book book) {
        if (book.getName() != null) db.update("update book set name = ? where id = ?", book.getName(), book.getId());
        if (book.getImageUrl() != null) db.update("update book set imageUrl = ? where id = ?", book.getImageUrl(), book.getId());
